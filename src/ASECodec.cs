...
namespace RD_AAOW
	{
	/// <summary>
	/// Класс описывает кодек для палитр типа Adobe Swatches Exchange
	/// </summary>
	public class ASECodec:IPaletteCodec
		{
		[DllImport (ProgramDescription.AssemblyCodecsLibrary)]
		private static extern Int16 ASE_LoadPalette (string FileName, out IntPtr Buffer, out UInt16 ColorsCount);	// RGB

		[DllImport (ProgramDescription.AssemblyCodecsLibrary)]
		private static extern Int16 ASE_SavePalette (string FileName, byte[] Buffer, UInt16 ColorsCount);			// RGB
		...
		public ProgramErrorCodes SavePalette (string FilePath, List<Color> Palette)
			...
			// Обращение
			return (ProgramErrorCodes)ASE_SavePalette (FilePath, array, (UInt16)Palette.Count);
			}
